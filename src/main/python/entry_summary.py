...
def tap_dance_rows(entry, label_fn, slot_labels=TAP_DANCE_SLOT_LABELS):
    ...
    """
    return [(name, _display(qmk_id, label_fn)) for name, qmk_id in zip(slot_labels, entry)]
...
def tap_dance_tooltip(entry, label_fn, index=None, slot_labels=TAP_DANCE_SLOT_LABELS,
    ...
    for name, value in tap_dance_rows(entry, label_fn, slot_labels):
        lines.append("{}: {}".format(name, value))
    if show_tapping_term:
        lines.append("Tapping term: {} ms".format(tapping_term(entry)))
    return "\n".join(lines)
...
def combo_tooltip(entry, label_fn, index=None):
    ...
    lines = []
    if index is not None:
        lines.append("Combo {}".format(index))
    inputs, output = combo_rows(entry, label_fn)
    lines.append("Keys: {}".format(COMBO_INPUT_SEPARATOR.join(inputs) if inputs else EMPTY))
    lines.append("Output: {}".format(output))
    return "\n".join(lines)
...
def build_entry_tooltips(entries, host_os_base, host_os_count, label_fn):
    ...
    for idx, entry in enumerate(entries):
        ...
        if host_os_base <= idx < host_os_end:
            ...
            tooltips[qmk_id] = tap_dance_tooltip(entry, label_fn)
    return tooltips
...
def macro_action_line(action, label_fn):
    ...
    if tag == "text":
        ...
        body = EMPTY
    return _shorten("{}: {}".format(tag.capitalize(), body))


def macro_lines(actions, label_fn, limit=MACRO_LINES):
    """The first `limit` actions of a macro as lines; an empty macro is a single EMPTY line."""
    if not actions:
        return [EMPTY]
    return [macro_action_line(a, label_fn) for a in list(actions)[:limit]]
