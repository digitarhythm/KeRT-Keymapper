# macOS 版 KeRT-Keymapper.app のビルド（Apple Silicon）
...
flowchart LR
    ...
    subgraph here["util/macos"]
        ...
        H2["KeRT-Keymapper.spec の datas"]
        H3["build.sh の sips + iconutil"]
    end
    F1 -. 同等 .-> H1
    F2 -. 同等 .-> H2
    F3 -. 同等 .-> H3
    here --> A["target/macos-arm64/kert-keymapper-mac-arm64.dmg"]
```
...
| 対象 | ビルド環境 | 生成物 |
|---|---|---|
| Apple Silicon | arm64 の Python（M1 以降の Mac、`macos-15` ランナー） | `target/macos-arm64/kert-keymapper-mac-arm64.dmg` |
...
flowchart LR
    ...
    A --> AA["Artifact: kert-keymapper-mac-arm64"]
    T["tag v* を push"] --> A
...
起動すると `KeRT-Keymapper` のプロセスが **2つ** 見えます。1つは GUI 本体、もう1つは `multiprocessing` のリソーストラッカー
（`KeRT-Keymapper -c from multiprocessing.resource_tracker import main`）で、これは正常です。
...
```bash
xattr -d com.apple.quarantine /path/to/KeRT-Keymapper.app
```
